...
def filter_classes(dataset: datasets.ImageFolder, allowed: Optional[list[str]]) -> datasets.ImageFolder:
    if not allowed:
        return dataset
    allowed_set = set(allowed)
    class_to_idx = {c: i for i, c in enumerate(dataset.classes) if c in allowed_set}
    samples = [(p, class_to_idx[cname],) for p, idx in dataset.samples for cname, i in dataset.class_to_idx.items() if i == idx and cname in allowed_set]
    dataset.samples = samples
    dataset.targets = [s[1] for s in samples]
    dataset.classes = [c for c in dataset.classes if c in allowed_set]
